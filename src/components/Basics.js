  ...
  return (
    <div className="react-basics  right-side-main">
      ...
      <section className="react-basics__npm">
        ...
        <p>Wait for the browser to open up and show you. <br/>
        'Ctrl+SHIFT+i' to access your developer tools on the web paged that just opened and check to solve the errors:
        you have deleted some files and you have to delete the connections to the files that are not there anymore.</p>
      </section>
      <CreateReactComponent/>
      <section className="react-basics__jsx">
        <h2>Writing JSX</h2>
        <p>Be prepared to write code in JSX, that is HTML with a twist and flavoured with JavaScript. Confusing at first, I know, but nothing simpler:
        similar semantic to HTML but by adding JavaScript context in within curly brackets <em>{"{"}like this{"}"}</em>.</p>
      </section>
    </div>
  );
}
